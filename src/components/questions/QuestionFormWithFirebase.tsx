...
export default function QuestionFormWithFirebase({
  questionType,
  onSubmit,
  initialData,
  subjectId,
  ...
  const [uploadStatuses, setUploadStatuses] = useState({
    ...
    optionImages: {} as Record<string, boolean>
  });
  ...
  const handleRemoveOption = (optionId: string) => {
    ...
    if (optionImagePreviews[optionId]) {
      ...
      setUploadStatuses(prev => {
        ...
        delete newStatuses.optionImages[optionId];
        return newStatuses;
      });
    }
  };
  ...
  const handleQuestionImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploadStatuses(prev => ({ ...prev, questionImage: true }));
    ...
    try {
      ...
      setUploadStatuses(prev => ({ ...prev, questionImage: false }));
    }
  };
  ...
  const handleExplanationImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Set uploading status
    setUploadStatuses(prev => ({ ...prev, explanationImage: true }));
    ...
    try {
      ...
      if (questionType === 'mcq') {
        setFormData(prev => ({
          ...prev,
          explanationImageUrl: result.imageUrl
        }));
      } else {
        setFormData(prev => ({
          ...prev,
          suggestedAnswerImageUrl: result.imageUrl
        }));
      }
      ...
      setUploadStatuses(prev => ({ ...prev, explanationImage: false }));
    }
  };
  ...
  const handleOptionImageChange = async (e: React.ChangeEvent<HTMLInputElement>, optionId: string) => {
    ...
    try {
      ...
      handleOptionChange(optionId, 'imageUrl', result.imageUrl);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const isAnyUploading = 
      uploadStatuses.questionImage || 
      uploadStatuses.explanationImage || 
      Object.values(uploadStatuses.optionImages).some(status => status);
    ...
    try {
      // All images should already be uploaded to Firebase Storage
  ...
  // Function to render the upload button label based on upload status
  const getUploadButtonLabel = (isUploading: boolean) => {
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center space-x-4">
          <Button
            ...
            disabled={uploadStatuses.questionImage}
          >
            {getUploadButtonLabel(uploadStatuses.questionImage)}
          </Button>
          <input
            type="file"
            ref={fileInputRef}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center space-x-4 mt-2">
                      <Button
                        ...
                        size="sm"
                        onClick={() => {
                          const fileInput = document.getElementById(`option-${option.id}-image`) as HTMLInputElement;
                          fileInput?.click();
                        }}
                        disabled={uploadStatuses.optionImages[option.id]}
                      >
                        {uploadStatuses.optionImages[option.id] ? 'Uploading...' : 'Upload Image'}
                      </Button>
                      <input
                        type="file"
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center space-x-4">
              <Button
                ...
                disabled={uploadStatuses.explanationImage}
              >
                {getUploadButtonLabel(uploadStatuses.explanationImage)}
              </Button>
              <input
                type="file"
